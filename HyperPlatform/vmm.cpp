...
typedef struct _CONTEXT_SWITCH_MON{
	ULONG_PTR   GuestRip;
	GpRegisters GuestGpReg[100];
	int			GuestGpRegIndex;
	ULONG		GuestExecTimes;
}CSMON, *PCSMON;


CSMON  g_CsMonitor[4][100] = { 0 };
int    g_MonitorPointer[4] = { 0 };
////////////////////////////////////////////////////////////////////////////////
//
// prototypes
//
...
// implementations
//
_Use_decl_annotations_ void __stdcall CSMonitorCallback(int ProcessorId, ULONG_PTR GuestRip, GpRegisters* GpReg, ULONG_PTR Cr3) 
{
	bool   IsFound	 = false;
	int	   index	 = g_MonitorPointer[ProcessorId];
	CSMON* CsMonitor = g_CsMonitor[ProcessorId];

	for (int i = 0; i < 100 ; i++)
	{
		CSMON* CsMon = &CsMonitor[i];
		if (CsMon->GuestRip == GuestRip) 
		{
			int bufferIndex = CsMon->GuestGpRegIndex;
			memcpy(&CsMon->GuestGpReg[bufferIndex] , GpReg, sizeof(GpRegisters));
			GpRegisters gp = CsMon->GuestGpReg[bufferIndex];

			HYPERPLATFORM_LOG_DEBUG("ContextSwitch at= %p ExecTimes= %x cr3= %p Esi= %p Esp= %p",CsMon->GuestRip, CsMon->GuestExecTimes, Cr3, gp.si, gp.sp);
			
			CsMon->GuestGpRegIndex = ((bufferIndex + 1) % 100);
			CsMon->GuestExecTimes++;
	...
	if (!IsFound) {
		CSMON* CsMon = &CsMonitor[index];
		CsMon->GuestRip = GuestRip;
		memcpy(&CsMon->GuestGpReg[0], GpReg, sizeof(GpRegisters));
		GpRegisters gp = CsMon->GuestGpReg[0];
		HYPERPLATFORM_LOG_DEBUG("[First Cap] ContextSwitch at= %p ExecTimes= %x cr3= %p Esi= %p Esp= %p", CsMon->GuestRip, CsMon->GuestExecTimes, Cr3, gp.si, gp.sp);
		CsMon->GuestGpRegIndex = 1;
		CsMon->GuestExecTimes = 1;
	}
	g_MonitorPointer[ProcessorId] = (index + 1) % 100;
}
